...
class StreamingCCRHandler:
    ...
    def _parse_sse_stream(self, data: bytes) -> dict[str, Any]:
        ...
        if self.provider == "anthropic":
            return self._anthropic_sse_envelope_from_events(events).message
        else:
    ...
    @staticmethod
    def _parse_anthropic_sse_envelope(data: bytes):
    ...
    @staticmethod
    def _anthropic_sse_envelope_from_events(events: list[dict[str, Any]]):
        """Build a request-local Anthropic envelope from decoded events."""

        from headroom.proxy.anthropic_wire import AnthropicSSEEnvelope

        return AnthropicSSEEnvelope.from_events(events)
